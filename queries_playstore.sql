...
ELECT COUNT(*) FROM analytics;

-- Find all the categories that have more than 300 apps in them.
SELECT category, count(*) FROM analytics GROUP BY category HAVING count(*) > 300;

-- Find the app that has the highest proportion of min_installs to reviews, among apps that have been installed at least 100,000 times. Display the name of the app along with the number of reviews, the min_installs, and the proportion.q
SELECT app_name, reviews, min_installs, reviews / min_installs AS proporton FROM analytics WHERE min_installs >=100000 ORDER by reviews/min_installs DESC LIMIT 1;
